...
const MIN_REQUEST_SPACING_MS=200; // ~5 req/s per runtime, safely below Onfleet's org-wide 20 req/s cap.
...
export async function requestOnfleet(path,{fetcher=fetch,key=process.env.ONFLEET_API_KEY}={}) {
  ...
  for(let attempt=0;attempt<=MAX_429_RETRIES;attempt+=1){
    ...
    if(Number.isFinite(remaining) && remaining<=2) addCooldown(1000);
...
async function fetchTasksById(ids,request,stopAt){
  ...
  // Intentionally sequential. Onfleet applies a 20 req/s limit across the whole organization.
  for(const id of ids){
    if(Date.now()>=stopAt){complete=false;break;}
    try{
...
export async function getOnfleet(date,{now=Date.now(),request=requestOnfleet,useCache=true}={}) {
  if(useCache && cached?.date===date && now-cached.at<45_000) return cached.value;
  ...
  async function load() {
    ...
    // Most active tasks were created recently. Fetch them in paginated batches first,
    // then individually fetch only current tasks that were created earlier than this window.
    ...
    const complete=currentIds.complete && olderCurrent.complete && done.complete;
    const warnings=[...currentIds.warnings,...olderCurrent.warnings];
    if(!recent.complete) warnings.push(`Recent active-task batch scan was partial${recent.error?`: ${recent.error.message}`:'.'}`);
